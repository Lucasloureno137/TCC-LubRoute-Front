...
export class FormUsuariosComponent implements OnInit {
  ...
  breadcrumbItems: BreadcrumbItem[] = [
    { label: 'Cadastros', },
    { label: 'Novo Usuario Admin', active: true }
  ];
  ...
  ngOnInit(): void {
    ...
    if (id_usuario && id_usuario !== 'app-form-usuarios') {
      this.isEdit = true;
  ...
  loadUsuario(id_usuario: string) {
    ...
    const apiUrl = `${baseUrl}/lubvel/user/${id_usuario}`;
  ...
  postData(): void {
    ...
    const apiUrl = `${baseUrl}/lubvel/user`;
    ...
    request.subscribe(
      (response) => {
        ...
        this.cleanData();
        this.router.navigate(['admin/app-lista-usuarios']);
      },
      (error) => {
